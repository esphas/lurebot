...
export class Auth {
    ...
    _calc_safe_calls() {
        const pick_fn = <T extends SafeCallPermission>(
            d: SafeCallDescMap[T],
        ) => {
            return {
        ...
        return Object.fromEntries(
            SAFE_CALL_PERMISSIONS.map((k) => [
                k,
                pick_fn(SAFE_CALL_DESC_MAP[k]),
            ]),
        ) as unknown as {
    ...
    get_safe_calls(user_id: number, scope_id: number) {
        ...
        const permissions = this.user_scope_role
            ...
            .filter((p) =>
                ['chat', 'trusted', 'moderate', 'root'].includes(p),
            ) as ('always' | 'chat' | 'trusted' | 'moderate' | 'root')[]
    ...
    admin() {
        ...
        const users = this.user_scope_role.find_users_with_role(
            scope.id,
            'admin',
        )
        if (users.length === 0) {
