...
import { Participant, ParticipantRepository } from './participant'
import { SessionVariable, SessionVariableRepository } from './variable'
...
export class Sessions {
    ...
    create_session(identifier: {
        ...
        user_id: number,
        scope_id: number,
    }, options: Partial<Session> = {}): Optional<Session> {
    ...
    get_participant_sessions(user_id: number, validate: boolean = true): Session[] {
        const sps = this.participant.select({ user_id })
        return sps.map((sp) => this.get_session(sp.session_id, validate)).filter((session) => session != null)
    }
