...
export class Agent {
  ...
  private listeners: Map<
    EventHandleMap[EventKey],
    [event: EventKey, fn: EventHandleMap[EventKey]]
  > = new Map();
  ...
  on<T extends EventKey>(event: T, handler: EventHandleMap[T], name?: string) {
    ...
    this.logger.log(
      "debug",
      `Registering event ${event} for agent ${this.file}${name ? ` (${name})` : ""}`,
    );
  ...
  off<T extends EventKey>(event: T, handler: EventHandleMap[T]) {
    if (this.listeners.has(handler)) {
      const [event, fn] = this.listeners.get(
        handler as EventHandleMap[EventKey],
      )!;
      this.app.napcat.off(event, fn);
      this.listeners.delete(handler);
    }
  ...
  async reload() {
    ...
    try {
      this.loaded = true;
      const core = (await import(`./${this.dir}/${this.file}?t=${Date.now()}`))
        .default;
