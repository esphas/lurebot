...
export const commands = [
  {
    ...
    handler: async (context, match) => {
      const endgame =
        match![1].trim() === "hangmanend" || match![1].trim() === "结束猜词";
      ...
      if (game == null) {
        ...
        if (game.revealed === game.word) {
          await context.reply(`你赢了！正确答案是 ${game.word}。${guessed}`);
          sessions.delete_session(session_id);
          return;
        }

        if (game.attempts >= max_attempts) {
          await context.reply(`你输了！正确答案是 ${game.word}。${guessed}`);
          sessions.delete_session(session_id);
          return;
        }
