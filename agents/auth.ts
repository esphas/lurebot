...
export const commands = [
    {
        ...
        handler: async (context) => {
            const result = context
                .auth('always')
                .auth.claim_admin(context.user.id)
            if (result) {
    ...
    } as Command<'message'>,
    {
        ...
        handler: async (context, match, app) => {
            ...
            const members = (
                ...
                context
                    .auth('always')
                    .user.from_napcat({ user_id: member.user_id }),
            )
            const users: User[] = []
            let success = 0
            if (is_add) {
                ...
                for (const member of members) {
                    users.push(
                        ...context.auth('root').user.unregister(member.id),
                    )
                }
                success = users.filter((user) => !user.registered).length
            }
