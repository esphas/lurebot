...
import { Database, Conflict, WhereCondition, QueryOptions, WhereValue } from './database'

export abstract class Repository<T, DB extends Record<keyof T, unknown> = T> {

    constructor(protected db: Database, protected logger: Logger, protected table: string) {
    }
    ...
    get tf() {
        return {
            ...
            date_or: <V>(null_value: V) => ({
                ...
                from_raw: (value: string) => value === null_value ? null : new Date(value)
            }),
            json: {
    ...
    raw(data: Partial<T>): Partial<DB> {
        ...
        for (const [key, t_value] of Object.entries(data) as [keyof T, T[keyof T]][]) {
            raw_data[key] = this.transform[key].raw(t_value)
        }
        (Object.entries(raw_data) as [keyof T, DB[keyof T]][]).forEach(([key, value]) => {
            if (value === undefined) {
                delete raw_data[key]
            }
        })
    ...
    from_raw(raw: DB): T {
        ...
        for (const [key, db_value] of Object.entries(raw) as [keyof T, DB[keyof T]][]) {
    ...
    raw_condition(condition: WhereCondition<T>): WhereCondition<DB> {
        ...
        for (const [key, t_value] of Object.entries(condition) as [keyof T, T[keyof T] | WhereValue<T[keyof T]>][]) {
            if (typeof t_value === 'object' && 'operator' in t_value! && 'value' in t_value) {
                const { operator, value } = t_value
                if (operator === 'in') {
                    raw_condition[key] = { operator, value: value.map(this.transform[key].raw) }
                } else if (operator === 'like') {
                    raw_condition[key] = { operator, value }
                } else if (operator === 'is null' || operator === 'is not null') {
                    raw_condition[key] = { operator, value: null }
                } else {
                    raw_condition[key] = { operator, value: this.transform[key].raw(value!) }
                }
            } else {
    ...
    select(condition: WhereCondition<T>, options: QueryOptions = {}): T[] {
        ...
        return this.db.select<DB>(this.table, raw_condition, options).map(this.from_raw.bind(this))
    }

    update(data: Partial<T>, condition: WhereCondition<T>): T[] {
        const raw_data = this.raw(data)
        const raw_condition = this.raw_condition(condition)
        return this.db.update<DB>(this.table, raw_data, raw_condition).map(this.from_raw.bind(this))
    }
    ...
    get(condition: WhereCondition<T>, options: QueryOptions = {}): T | undefined {
        const raw_condition = this.raw_condition(condition)
    ...
    upsert(data: Partial<T>, condition: WhereCondition<T>): T {
        const raw_data = this.raw(data)
        const raw_condition = this.raw_condition(condition)
        return this.from_raw(this.db.upsert<DB>(this.table, raw_data, raw_condition))
    }
}
