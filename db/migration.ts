...
export class MigrationManager {
    ...
    run_migrations(migrations: Migration[]) {
        const applied_versions = this.db
            .prepare('select version from migrations order by version')
            .all() as { version: number }[]
        const applied_versions_set = new Set(
            applied_versions.map((v) => v.version),
        )
    ...
    show_migration_history() {
        ...
        for (const migration of results) {
            this.log(
                'info',
                `  v${migration.version} - ${migration.applied_at}`,
            )
        }
    }
    ...
    check_migrations(migrations: Migration[]) {
        ...
        const pending_migrations = migrations.filter(
            (m) => !applied_versions_set.has(m.version),
        )

        if (pending_migrations.length > 0) {
            this.log(
                'info',
                `待应用的迁移: ${pending_migrations.map((m) => m.version).join(', ')}`,
            )
