...
export class Database {
    ...
    private build_where_clause<T>(condition: WhereCondition<T>): {
        sql: string
        params: unknown[]
    } {
    ...
    select<T>(
        ...
        const { sql, params } = this.build_select_query(
            table,
            condition,
            options,
        )
    ...
    insert<T>(
        ...
        if (result.changes > 0) {
            ...
            } as unknown as WhereCondition<T>)[0] as T
        }
        return undefined
    }
    ...
    update<T>(
        ...
        const where_clause = this.build_where_clause(condition)
        const set_clause = Object.keys(data)
            .map((key) => `${key} = ?`)
            .join(', ')
    ...
    get<T>(
        ...
        const { sql, params } = this.build_select_query(table, condition, {
            ...options,
            limit: 1,
        })
    ...
    count<T>(table: string, condition: WhereCondition<T>): number {
        ...
        const result = this.db.prepare(sql).get(...where_clause.params) as {
            count: number
        }
        return result.count
    }
